...
def get_cars():
    ...
    if sort_by is None or sort_by == "":
        return jsonify({"error": "Missing or invalid 'sort_by' parameter"}), 400

    if limit is None or limit == "":
        return jsonify({"error": "Missing or invalid 'limit' parameter"}), 400

    if sort_by not in cars_db[next(iter(cars_db))]:
    ...
    sorted_cars = sorted(cars_db.values(), key=lambda x: x.get(sort_by, 0))
    limited_cars = sorted_cars[:limit] if limit else sorted_cars
